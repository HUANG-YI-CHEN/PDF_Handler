...
namespace PDF_Handler
{
    public class PdfConverter : IDisposable
    {
        ...
        private void Pdf2Word(string srcPath, string dstPath)
        {
            PdfDocument doc = new PdfDocument();
            //PdfPageNumber startPage = new PdfPageNumber();
            //startPage.StartNumber = 9;
            //doc.PageNumber = startPage;
            doc.LoadFromFile(srcPath);
            doc.SaveToFile(dstPath, FileFormat.DOC);
            System.Diagnostics.Process.Start(dstPath);
        }
